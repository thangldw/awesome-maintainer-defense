...
- Consolidated the offline auditor, reversible profiles, policy assets, evidence corpus and reviewed catalog into `1.0.0`.
- Kept deployable sample workflows as product data while removing workflows that executed in this repository.
- Standardized package and release metadata at `v1.0.0`.

### Tiếng Việt

- Hợp nhất auditor offline, profile có thể hoàn tác, policy asset, evidence corpus và catalog đã review vào `1.0.0`.
- Giữ workflow mẫu dưới dạng dữ liệu sản phẩm nhưng xóa workflow chạy trong chính repo.
- Chuẩn hóa package và release metadata thành `v1.0.0`.

### 日本語

- オフライン監査、復元可能なプロファイル、ポリシー資産、証拠コーパス、レビュー済みカタログを `1.0.0` に統合しました。
